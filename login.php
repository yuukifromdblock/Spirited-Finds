...
// Kung nakadikit na ang Session at naka-login na, i-redirect pabalik sa index.php
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($email) && !empty($password)) {
        ...
        if ($result && $result->num_rows === 1) {
            ...
            // Verifying password (suportado ang hashed at plain text)
            if (password_verify($password, $user['password']) || $password === $user['password']) {
                ...
                // Pag-redirect batay sa Role
                if ($user['role'] === 'admin') {
                    header("Location: index.php"); // Pwedeng palitan ng admin_dashboard.php balang araw
                } else {
                    header("Location: index.php");
                }
                exit();
            } else {
                $error_msg = "Maling password! Paki-subok ulit.";
            }
        } else {
            $error_msg = "Walang account na nakarehistro sa email na ito.";
        }
        $stmt->close();
    } else {
        $error_msg = "Paki-sagutan ang lahat ng larangan.";
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Login - Spirited Finds</title>
...
<div class="login-container">
    <h3 class="login-header">Welcome back to the world of Spirited Finds! Let's embark on another magical adventure.</h3>
    ...
    <p class="quote">"The wind is rising! ... We must try to live!" - Paul Valéry</p>
    <p class="text-center mt-3" style="color: #6f6f6f;">Don't have an account? <a href="signup.php" style="color: #ffd700;">Sign up</a></p>
</div>
